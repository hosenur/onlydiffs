...
const CommitAllRequest = Schema.Struct({ message: Schema.String });
...
export function registerIpcHandlers(): void {
  ...
  handle(IpcChannel.commitAll, CommitAllRequest, (request) =>
    Effect.flatMap(Diff, (diff) => diff.commitAll(request.message)),
  );

  handleNoInput(IpcChannel.claudeStatus, () =>
    Effect.flatMap(ClaudeChannel, (channel) => channel.status),
  );
