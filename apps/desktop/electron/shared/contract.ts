...
export const IpcChannel = {
  ...
  commitAll: "onlydiffs:commit-all",
  claudeStatus: "onlydiffs:claude-status",
} as const;
...
export interface OnlyDiffsApi {
  ...
  /**
   * Sends a message into the Claude Code session for the open repository.
   * One-way: resolves with the channel's message id once handed over, and
   * nothing waits for Claude to respond.
   */
  sendClaudeMessage(
  ...
  /** Stages everything and commits it. Resolves with the new HEAD line. */
  commitAll(request: CommitAllRequest): Promise<IpcResult<string>>;
  /** Whether a Claude Code session is listening for the open repository. */
  claudeStatus(): Promise<IpcResult<ClaudeChannelStatus>>;
}
